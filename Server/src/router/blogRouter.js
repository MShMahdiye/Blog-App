...
router.post('/write', async (req, res, next) => {
  ...
  try {
    ...
    if (!req.body.title || !req.body.content || !req.body.imgurl) return res.status(522).json({ msg: 'bad request' })
...
router.post('/by-user', async (req, res, next) => {
  try {
    ...
    if (!thisUser || !thisUser._id) return res.status(500).json({ msg: 'bad request' })
